...
The `mtp.stat` class provides information about filename, timestamp and size
of files on a portable device.
An instance of `mtp.stat` is generated by calling `path.dir()` and `path.walk()` with
a `directory` instance created by `mtp.storage#opendir()` for the argument and `:stat` attribute appended.
