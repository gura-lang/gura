...
bool Gear_Relu::DoDirProp(Environment &env, SymbolSet &symbols)
{
	return Gear::DoDirProp(env, symbols);
}

Value Gear_Relu::DoGetProp(Environment &env, const Symbol *pSymbol, const SymbolSet &attrs, bool &evaluatedFlag)
{
...
bool NodeGear_Relu::EvalBackward(Environment &env)
{
	...
	if (_connectorSrc.GetNodeSrc()->IsVulnerable()) {
		if (!Array::Mul(env, _connectorSrc.GetArrayGradAutoPtr(),
						_pArrayBool.get(), (*ppConnectorDst)->GetArrayGrad())) return false;
	}
	return true;
}
