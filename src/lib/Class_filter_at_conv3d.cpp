...
Array *Filter_Conv3d::Apply(Signal &sig, Array *pArrayResult, const Array *pArray)
{
	FilterFuncT filterFunc = filterFuncTable.funcs[pArray->GetElemType()][Array::ETYPE_None];
	if (filterFunc == nullptr) {
		sig.SetError(ERR_TypeError, "can't apply 3-dimension convolution filter on this array");
		return nullptr;
	}
...
//-----------------------------------------------------------------------------
Object_filter_at_conv3d::Object_filter_at_conv3d(Environment &env, const Filter_Conv3d &filter) :
	Object(env.LookupClass(VTYPE_filter_at_conv3d)), _filter(filter)
{
}
...
{
	Object_filter_at_conv3d *pObj = new Object_filter_at_conv3d(env, Filter_Conv3d());
