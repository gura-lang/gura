...
{
	SetMode(RSLTMODE_Normal, FLAG_None);
	AddHelp(Gura_Symbol(en), SYN_markdown, "Returns the length of the buffer in binary.");
}
...
Gura_DeclareMethod(binary, each)
{
	SetMode(RSLTMODE_Normal, FLAG_None);
	DeclareBlock(OCCUR_ZeroOrOnce);
	AddHelp(Gura_Symbol(en), SYN_markdown, "Returns an iterator picking up each byte in the buffer");
}
