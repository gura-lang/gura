...
public:
	inline wx_MiniFrame() : wxMiniFrame(), _pSig(nullptr), _pObj(nullptr) {}
	inline wx_MiniFrame(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style, const wxString& name) : wxMiniFrame(parent, id, title, pos, size, style, name), _pSig(nullptr), _pObj(nullptr) {}
	~wx_MiniFrame();
