...
public:
	inline wx_RichTextListStyleDefinition(const wxString& name) : wxRichTextListStyleDefinition(name), _pSig(nullptr), _pObj(nullptr) {}
...
Gura_ImplementMethod(wx_RichTextListStyleDefinition, CombineWithParagraphStyle)
{
	...
	return ReturnValue(env, sig, args, Value(new Object_wx_RichTextAttr(new wxRichTextAttr(rtn), nullptr, OwnerTrue)));
}
...
Gura_ImplementMethod(wx_RichTextListStyleDefinition, GetLevelAttributes)
{
	...
	return ReturnValue(env, sig, args, Value(new Object_wx_RichTextAttr(rtn, nullptr, OwnerFalse)));
}
