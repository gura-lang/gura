...
private:
	...
	Object_wx_URL *_pObj;
public:
	inline wx_URL(const wxString& url) : wxURL(url), _pSig(nullptr), _pObj(nullptr) {}
	~wx_URL();
...
Gura_ImplementMethod(wx_URL, GetProtocol)
{
	...
	return ReturnValue(env, sig, args, Value(new Object_wx_Protocol(new wxProtocol(rtn), nullptr, OwnerTrue)));
#endif
