...
Gura_ImplementFunction(Menu)
{
	...
	Object_wx_Menu *pObj = Object_wx_Menu::GetObjectThis(args);
	if (pObj == nullptr) {
...
{
	Signal &sig = env.GetSignal();
	Object_wx_Menu *pThis = Object_wx_Menu::GetObjectThis(args);
...
Gura_ImplementMethod(wx_Menu, GetMenuItems)
{
	...
	return ReturnValue(env, args, MenuItemListToValue(env, rtn));
}
...
Value MenuItemListToValue(Environment &env, const wxMenuItemList &list)
{
	...
	foreach_const (wxMenuItemList, pItem, list) {
		valList.push_back(Value(new Object_wx_MenuItem(*pItem, nullptr, OwnerFalse)));
	}
	return rtn;
}
