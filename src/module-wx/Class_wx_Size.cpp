...
bool Object_wx_Size::DoDirProp(Environment &env, Signal &__to_delete__, SymbolSet &symbols)
{
...
Value Object_wx_Size::DoGetProp(Environment &env, Signal &__to_delete__, const Symbol *pSymbol,
...
Value Object_wx_Size::DoSetProp(Environment &env, Signal &__to_delete__, const Symbol *pSymbol, const Value &value,
						const SymbolSet &attrs, bool &evaluatedFlag)
{
	Signal &sig = GetSignal();
	if (pSymbol->IsIdentical(Gura_Symbol(x))) {
