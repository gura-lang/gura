...
Gura_ImplementFunction(TCPClientEmpty)
{
	...
	Object_wx_TCPClient *pObj = Object_wx_TCPClient::GetObjectThis(arg);
	if (pObj == nullptr) {
...
Gura_ImplementMethod(wx_TCPClient, MakeConnection)
{
	...
	wxString host = wxString::FromUTF8(arg.GetString(0));
	wxString service = wxString::FromUTF8(arg.GetString(1));
	wxString topic = wxString::FromUTF8(arg.GetString(2));
